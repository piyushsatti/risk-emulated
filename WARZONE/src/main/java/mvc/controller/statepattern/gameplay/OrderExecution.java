...
public class OrderExecution extends Phase {
    ...
    public void run() {
        ...
        if (isWinner() || (d_ge.getNumberOfTurns() == Counter)) {
            ...
            if (isWinner()) {
                d_ge.setGameResult(d_ge.getNumberOfGames() - 1, d_ge.d_currentMapIndex, d_ge.d_players.get(0).getName());
            } else {
                d_ge.setGameResult(d_ge.getNumberOfGames() - 1, d_ge.d_currentMapIndex, "DRAW");
            }
