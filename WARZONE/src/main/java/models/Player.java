...
public class Player {
   ...
   /* public void issue_order() throws InvalidCommandException {
        ...
        while (true) {
            ...
            int l_countryID = GameEngine.map.getCountryID(arr[1]);
            ...
            if (l_countryID > 0 && l_numberTobeDeployed <= this.getReinforcements()) {
                ...
                if (this.d_assignedCountries.contains(l_countryID)) {
                    ...
                    TerminalRenderer.renderMessage("Order Created. Here are the Details: Deploy " + l_numberTobeDeployed + " on " + GameEngine.map.getCountry(l_countryID).getCountryName() + " by Player: " + this.d_playerName);
